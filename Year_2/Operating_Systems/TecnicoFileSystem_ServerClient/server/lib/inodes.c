...
int inode_create(uid_t owner, permission ownerPerm, permission othersPerm){
    ...
    for(int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++){
        if(inode_table[inumber].owner == FREE_INODE){
            ...
            inode_table[inumber].connectedClients = 0;
            unlock_inode_table();
            return inumber;
        }
    }
...
/*
 * increments or decrements connectedClients counter
 * Input:
 *  - inumber: identifier of the i-node
 *  - key: i -> increments / d -> decrements / r -> reads the counter
 * Returns:
 *    number of connected clients: if successful
 *   -1: if an error occurs
 */
int inode_clientCounter(int inumber, char key){
    lock_inode_table();
    if(inumber < 0 || (key != 'i' && key != 'd' && key != 'r')){
        printf("inode_clientCounter: invalid inumber");
        unlock_inode_table();
        return -1;
    }
    ...
    switch(key){
        case 'i':
            connected_clients = ++inode_table[inumber].connectedClients;
            break;
        case 'd':
            connected_clients = --inode_table[inumber].connectedClients;
            break;
