...
typedef struct {
    ...
    node_t hop_arr[MAX_NODES][MAX_NODES];
} state_t;
...
void bellman_ford(node_t curr_node, state_t* curr_state) {
    ...
    for (node_t dest_node = get_first_node(); dest_node <= get_last_node(); dest_node++) {
        ...
        if (curr_state->table[curr_node][dest_node] != min_cost) {
            int i;
            for (i = 0; curr_state->hop_arr[hop_select][i] != -1 && i <= get_last_node(); i++);
            curr_state->hop_arr[hop_select][i] = dest_node;

            curr_state->table[curr_node][dest_node] = min_cost;
            set_route(dest_node, hop_select, min_cost);
            alter_flag = 1;
        }
        else if (curr_state->table[curr_node][dest_node] == min_cost) {
             set_route(dest_node, hop_select, min_cost);
        }
    }
    ...
    if (alter_flag) {
        for (node_t neighbor = get_first_node(); neighbor <= get_last_node(); neighbor++) {
            if (get_link_cost(neighbor) < COST_INFINITY && neighbor != curr_node) {
                ...
                for (int i = 0; curr_state->hop_arr[neighbor][i] != -1 && i <= get_last_node(); i++) {
                    msg->arr[curr_state->hop_arr[neighbor][i]] = COST_INFINITY;
                }
...
void notify_link_change(node_t neighbor, cost_t new_cost) {
    ...
    if (!curr_state) {
        ...
        for (int i = 0; i <= get_last_node(); i++)
            for (int j = 0; j <= get_last_node(); j++)
                curr_state->hop_arr[i][j] = -1;
...
void notify_receive_message(node_t sender, void *message) {
    state_t *curr_state = (state_t *) get_state();
    cost_t *recv_data = ((message_t *)message)->arr;

    memcpy(curr_state->table[sender], recv_data, sizeof(cost_t) * MAX_NODES);
